...
class Cell:
    ...
    def __sub__(self, other):
        if self.__count > other.__count:
            return Cell(self.__count - other.__count)
        print("Разность количества ячеек клеток должна быть больше 0!")
    ...
    def __str__(self):
        return f"Количество ячеек: {self.__count}"

    def make_order(self, num):
        pref = '' if self.__count < num else '\n'.join('*' * num for _ in range(self.__count//num)) + '\n'
        return pref + '*' * (self.__count % num) if self.__count % num else pref[:-1]
